import { computed, useCreateStore } from 'jastaman';
import { useRef } from 'react';
import { isInteger } from '../../columnType.utils';

export const useNumberInputStore = () => {
  const inputRef = useRef<HTMLInputElement>(null);

  const store = useCreateStore(() => ({
    inputRef,
    state: {
      value: '',
      isRaw: false,
      type: '',
      default: undefined as string | undefined,
      isNullable: undefined as boolean | undefined,
      isInteger: computed<boolean>(),
      isValid: computed<boolean>(),
      placeholder: computed<string>(),
    },
    computed: {
      isInteger: [(state) => [state.type], (state) => isInteger(state.type)],
      isValid: [
        (state) => [state.value, state.isRaw, state.isInteger],
        ({ isRaw, value, isInteger }) => {
          return isRaw
            ? true
            : !isNaN(isInteger ? parseInt(value) : parseFloat(value));
        },
      ],
      placeholder: [
        (state) => [state.value, state.isRaw, state.default, state.isNullable],
        (state) => {
          return state.value === null || state.isRaw
            ? state.default || (!state.isNullable ? 'required' : 'null')
            : 'empty';
        },
      ],
    },
    init(data: {
      value: string;
      isRaw: boolean;
      type: string;
      default: string | undefined;
      isNullable: boolean | undefined;
    }) {
      store.set(data);
    },
    setValue(value: string) {
      store.set({ value });
    },
    setIsRaw(isRaw: boolean) {
      store.set({ isRaw });
    },
    getIsValid() {
      return store.state.isValid;
    },
    getParsedValue() {
      const { value, isRaw, isInteger } = store.state;
      return isRaw
        ? value
        : String(isInteger ? parseInt(value) : parseFloat(value));
    },
  }));

  return store;
};
